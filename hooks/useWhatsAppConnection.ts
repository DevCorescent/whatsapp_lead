...
export function useWhatsAppSignupConfig() {
  return useQuery<EmbeddedSignupConfigDTO>({
    ...
    queryFn: async () => {
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(json?.error ?? "Could not load the connection settings");
...
export function useConnectWhatsApp() {
  ...
  return useMutation({
    mutationFn: (input: ConnectWhatsAppInput) =>
      postJson<{ connection: WhatsAppConnectionDTO; warnings: string[] }>(
        "/api/integrations/whatsapp/connect",
        input,
        "Could not complete the WhatsApp connection",
      ),
    onSuccess: () => {
      // The business rows now carry a number and a token; every screen that shows
      // connection state reads them from this one query.
