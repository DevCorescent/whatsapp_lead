...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log("[admin/stats] fetching data for SUPER_ADMIN:", session.user.email);
    ...
    console.log("[admin/stats] raw counts — tenants:", totalTenants, "users:", totalUsers, "messages:", totalMessages);

    const mrr = activeSubscriptions.reduce((s, sub) => s + (sub.plan?.priceMonthly ?? 0), 0);
    ...
    for (const sub of activeSubscriptions) {
      planNames[sub.planId] = sub.plan?.displayName ?? sub.plan?.name ?? sub.planId;
    }
    ...
    console.log("[admin/stats] success — mrr:", mrr, "churnRate:", churnRate, "planBreakdown:", JSON.stringify(planBreakdown));
